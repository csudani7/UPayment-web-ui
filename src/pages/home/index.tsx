...
function HomePage() {
  ...
  const { data: listOfProducts, isLoading } = useGetAllProducts();
  const [selected, setSelected] = React.useState({ id: 0, name: 'Categories' });
  const [inputValue, setInputValue] = React.useState('');

  const filterdData = React.useMemo(() => {
    let products: GetAllProductResponse[] | undefined = listOfProducts && [...listOfProducts];
    if (inputValue) {
